        ...
        if (label.equalsIgnoreCase("stomper")) {
            if (!KitManager.usandokit.contains(p.getName())) {
                if (p.hasPermission("kit.stomper")) {
                    ...
                    p.sendMessage(ChatColor.translateAlternateColorCodes('&', Stomper.plugin.getConfig().getString("Sem_Permiss\u00e3o_Kit")));
                }
            }
            else {
                p.sendMessage(ChatColor.translateAlternateColorCodes('&', Stomper.plugin.getConfig().getString("Um_Kit_Por_Vida")));
            }
            return true;
        }
        return false;
