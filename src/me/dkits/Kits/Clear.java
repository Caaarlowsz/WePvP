        ...
        if (label.equalsIgnoreCase("clear")) {
            if (!KitManager.usandokit.contains(p.getName())) {
                if (p.hasPermission("kit.clear")) {
                    ...
                    p.sendMessage(ChatColor.translateAlternateColorCodes('&', Clear.plugin.getConfig().getString("Sem_Permiss\u00c3£o_Kit")));
                }
            }
            else {
                p.sendMessage(ChatColor.translateAlternateColorCodes('&', Clear.plugin.getConfig().getString("Um_Kit_Por_Vida")));
            }
            return true;
        }
        return false;
