...
public class Bazooka implements Listener, CommandExecutor
{
    ...
    public void onGun(final PlayerInteractEvent e) {
        ...
        if ((!Bazooka.cooldown.containsKey(p.getName()) || Bazooka.cooldown.get(p.getName()) <= System.currentTimeMillis()) && (e.getAction() == Action.RIGHT_CLICK_AIR || e.getAction() == Action.RIGHT_CLICK_BLOCK) && KitManager.bazooka.contains(p.getName()) && p.getItemInHand().getType() == Material.GOLD_BARDING) {
            ...
            if (blocksToAdd.hasNext()) {
                ...
                h.setMetadata("bazooka", (MetadataValue)new FixedMetadataValue((Plugin)Bazooka.plugin, (Object)true));
                Bazooka.cooldown.put(p.getName(), System.currentTimeMillis() + TimeUnit.SECONDS.toMillis(15L));
                return;
            }
            p.sendMessage(ChatColor.RED + "Carregando em " + TimeUnit.MILLISECONDS.toSeconds(Bazooka.cooldown.get(p.getName()) - System.currentTimeMillis()) + " segundos para poder usar novamente.");
        }
    }
