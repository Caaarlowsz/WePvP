...
public class Kyuubi implements Listener, CommandExecutor
{
        ...
        if (label.equalsIgnoreCase("kyuubi")) {
            if (!KitManager.usandokit.contains(p.getName())) {
                if (p.hasPermission("kit.kyuubi")) {
                    ...
                    p.sendMessage(ChatColor.translateAlternateColorCodes('&', Kyuubi.plugin.getConfig().getString("Sem_Permiss\u00c3£o_Kit")));
                }
            }
            else {
                p.sendMessage(ChatColor.translateAlternateColorCodes('&', Kyuubi.plugin.getConfig().getString("Um_Kit_Por_Vida")));
            }
            return true;
        }
        return false;
    ...
    public void onPlayerInteract2(final PlayerInteractEvent e) {
        ...
        if ((e.getAction() == Action.RIGHT_CLICK_AIR || e.getAction() == Action.RIGHT_CLICK_BLOCK) && KitManager.kyuubi.contains(p.getName()) && p.getItemInHand().getType() == Material.BLAZE_POWDER) {
            if (!Kyuubi.cooldown2.containsKey(p.getName()) || Kyuubi.cooldown2.get(p.getName()) <= System.currentTimeMillis()) {
                e.setCancelled(true);
                p.updateInventory();
            ...
            p.sendMessage(ChatColor.RED + "Faltam " + TimeUnit.MILLISECONDS.toSeconds(Kyuubi.cooldown2.get(p.getName()) - System.currentTimeMillis()) + " segundos para poder usar sua habilidade de defesa novamente!");
        }
    }
    ...
    public void onPlayerInteract(final PlayerInteractEvent e) {
        final Player p = e.getPlayer();
        if ((e.getAction() == Action.RIGHT_CLICK_AIR || e.getAction() == Action.RIGHT_CLICK_BLOCK) && KitManager.kyuubi.contains(p.getName()) && p.getItemInHand().getType() == Material.NETHER_STAR) {
            if (!Kyuubi.cooldown.containsKey(p.getName()) || Kyuubi.cooldown.get(p.getName()) <= System.currentTimeMillis()) {
                e.setCancelled(true);
                p.updateInventory();
            ...
            p.sendMessage(ChatColor.RED + "Faltam " + TimeUnit.MILLISECONDS.toSeconds(Kyuubi.cooldown.get(p.getName()) - System.currentTimeMillis()) + " segundos para poder usar sua habilidade de defesa novamente!");
        }
    }
