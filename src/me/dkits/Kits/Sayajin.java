...
public class Sayajin implements CommandExecutor, Listener
{
        ...
        if (label.equalsIgnoreCase("sayajin")) {
            if (!KitManager.usandokit.contains(p.getName())) {
                if (p.hasPermission("kit.sayajin")) {
                    ...
                    p.sendMessage(ChatColor.translateAlternateColorCodes('&', Sayajin.plugin.getConfig().getString("Sem_Permiss\u00c3£o_Kit")));
                }
            }
            else {
                p.sendMessage(ChatColor.translateAlternateColorCodes('&', Sayajin.plugin.getConfig().getString("Um_Kit_Por_Vida")));
            }
            return true;
        }
        return false;
    ...
    public void onInteractPhantom(final PlayerInteractEvent event) {
        ...
        if (event.getPlayer().hasPermission("kit.sayajin") && event.getAction().name().contains("RIGHT") && event.getPlayer().getItemInHand().getType() == Material.GOLD_NUGGET && KitManager.sayajin.contains(event.getPlayer().getName())) {
            ...
            final int i = 5;
            Bukkit.getServer().getScheduler().scheduleSyncDelayedTask((Plugin)Sayajin.plugin, (Runnable)new Runnable() {
                @Override
                public void run() {
                    ...
                    if (blocksToAdd.hasNext()) {
                        ...
                        p.getWorld().playEffect(blockToAdd, Effect.STEP_SOUND, (Object)Material.GOLD_INGOT, 20);
                        p.getWorld().playEffect(blockToAdd, Effect.STEP_SOUND, (Object)Material.GOLD_NUGGET, 20);
