...
public class Endermage implements Listener, CommandExecutor
{
    ...
    public void EndermageInteract(final PlayerInteractEvent e) {
        ...
        if (e.getAction() == Action.RIGHT_CLICK_BLOCK && mage.getItemInHand().getType() == Material.PORTAL && KitManager.endermage.contains(mage.getName())) {
            e.setCancelled(true);
                ...
                new BukkitRunnable() {
                    ...
                    public void run() {
                        ...
                        if (!target.getInventory().contains(Material.PORTAL) && Endermage.this.isEnderable(bLoc, target.getLocation()) && target != mage && !target.isDead()) {
                            ...
                            if (!mage.getInventory().contains(new ItemStack(Material.PORTAL)) && KitManager.endermage.contains(mage.getName())) {
