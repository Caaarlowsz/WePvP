...
public class mlgvoltar implements Listener
{
    ...
    public void inv(final PlayerInteractEvent e) {
        ...
        if (e.getAction() == Action.RIGHT_CLICK_BLOCK && e.getClickedBlock() != null && (e.getClickedBlock().getType() == Material.WALL_SIGN || e.getClickedBlock().getType() == Material.SIGN_POST)) {
            ...
            if (lines.length > 2 && lines[1].equals("§6VOLTAR!") && lines.length > 1 && lines[0].equals("§4§o<---()--->")) {
                p.chat("/mlg");
            }
        }
    }
    ...
    public void inv2(final PlayerInteractEvent e) {
        final Player p = e.getPlayer();
        if (e.getAction() == Action.RIGHT_CLICK_BLOCK && e.getClickedBlock() != null && (e.getClickedBlock().getType() == Material.WATER || e.getClickedBlock().getType() == Material.WATER)) {
            p.sendMessage("§bN\u00e3o deixe sua \u00e1gua no ch\u00e3o, isso \u00e9 um bug e caso seja pego o fazendo, levar\u00e1 tempban! §fBom Jogo");
        }
    }
}
